...
class BookingServiceTests {
    ...
    void ifItemToBookBelongsToUserThenItemNotFoundException() {
        ...
        final ItemNotFoundException exception = Assertions.assertThrows(
                ItemNotFoundException.class,
                () -> bookingService.create(1L, bookingStartDto));
        Assertions.assertEquals("Пользователь попытался забронировать свою вещь", exception.getMessage());
    ...
    void ifTryToBookNotAvailableItemThenBadRequestException() {
        ...
        Assertions.assertEquals("Забронировать вещь не удалось из-за некорректных временных рамок", exception.getMessage());
        Mockito.verify(itemRepository, Mockito.times(1))
                .findById(Mockito.anyLong());
        Mockito.verify(userRepository, Mockito.times(1))
                .findById(Mockito.anyLong());
    ...
    @Test
    void ifBookingStartIsBeforeNowThenBadRequestException() {
        BookingStartDto bookingStartDto = BookingStartDto.builder()
    ...
    @Test
    void ifBookingStartIsAfterNowThenBadRequestException() {
        BookingStartDto bookingStartDto = BookingStartDto.builder()
    ...
    @Test
    void ifUserBookingStateIsIncorrectThenBadRequestException() {
        final BadRequestException exception = Assertions.assertThrows(
                BadRequestException.class,
    ...
    void ifUserBookingFromIsIncorrectThenIllegalPaginationException() {
        ...
        final IllegalPaginationException exception = Assertions.assertThrows(
    ...
    void ifGetUserBookingsAndFutureStateThenFindAllByBookerIdAndStartAfterBookingRepository() {
        ...
        Mockito.when(userRepository.existsById(1L))
                .thenReturn(true);
        Mockito.when(bookingRepository.findAllByBookerIdAndStartAfter(Mockito.anyLong(), Mockito.any(LocalDateTime.class),
                        Mockito.any(Pageable.class)))
                .thenReturn(bookings);
    ...
    void ifGetUserBookingsAndPastStateThenFindAllByBookerIdAndStartAfterBookingRepository() {
        ...
        Mockito.when(userRepository.existsById(1L))
                .thenReturn(true);
        Mockito.when(bookingRepository.findAllByBookerIdAndEndBefore(Mockito.anyLong(), Mockito.any(LocalDateTime.class),
                        Mockito.any(Pageable.class)))
                .thenReturn(bookings);
    ...
    @Test
    void ifUserBookingsFoundWithStateIsNotValidThenBadRequestException() {
        final BadRequestException exception = Assertions.assertThrows(
                BadRequestException.class,
                () -> bookingService.findBookingsByOwner(1L, "NONE", 0, 10));
    ...
    void ifBookingsOfOwnerFoundWithCurrentStateThenFindAllByBookerIdBookingRepository() {
        ...
        Mockito.verify(itemRepository, Mockito.times(1))
                .existsByOwnerId(Mockito.anyLong());
        Mockito.verify(bookingRepository, Mockito.times(1))
                .findAllCurrentByOwnerId(Mockito.anyLong(), Mockito.any(LocalDateTime.class), Mockito.any(Pageable.class));
        Mockito.verify(bookingRepository, Mockito.never())
                .findAllByOwnerId(Mockito.anyLong(), Mockito.any(Pageable.class));
        Mockito.verify(bookingRepository, Mockito.never())
                .findAllFutureByOwnerId(Mockito.anyLong(), Mockito.any(LocalDateTime.class), Mockito.any(Pageable.class));
        Mockito.verify(bookingRepository, Mockito.never())
