...
class ItemControllerTests {
    ...
    @Test
    void createItemWithEmptyDescriptionAndStatusIsBadRequest() throws Exception {
        createUser(owner);
        ItemDto description = ItemDto.builder()
                .name("велосипед")
                .description("")
                .available(true)
                .build();
        ...
        mockMvc.perform(post("/items")
                        ...
                        .accept(MediaType.APPLICATION_JSON)
                        .header("X-Sharer-User-Id", 1L)
                        .content(mapper.writeValueAsString(description)))
                .andExpect(status().isBadRequest());
    }

    @Test
    void createItemNotAvailableAndStatusIsBadRequest() throws Exception {
        createUser(owner);
        ItemDto description = ItemDto.builder()
                .name("велосипед")
                .description("женский")
    ...
    @Test
    void postBlankCommentStatusIsBadRequest() throws Exception {
        createUser(owner);
        createItem(itemDto, 1L);
